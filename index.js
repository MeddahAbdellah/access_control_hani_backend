...
app.get('/getControlData',(req,res)=>{
  ...
  connection.query('SELECT users.name,users.surname,controldata.key,controldata.date,controldata.valid From users RIGHT JOIN controldata ON controldata.key = users.key where controldata.date BETWEEN "'+selectDate+' 00:00:00" AND "'+selectDate+' 23:59:59 ORDER BY UNIX_TIMESTAMP(controldata.date) DESC"', function (error, results, fields) {
